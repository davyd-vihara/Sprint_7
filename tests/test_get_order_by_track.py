import pytest
import allure

from helpers.assertions import assert_insufficient_data_error, assert_order_not_found_error


class TestGetOrderByTrack:
    """Тесты для получения заказа по номеру"""
    
    @pytest.mark.order
    @allure.title("Получение заказа по номеру")
    @allure.description("Проверка успешного получения заказа по номеру")
    def test_get_order_by_track_success(self, api_client, created_order):
        """Проверка успешного получения заказа по номеру"""
        track = created_order["track"]
        
        with allure.step("Получить заказ по номеру"):
            get_response = api_client.get_order_by_track(track)
        
        with allure.step("Проверить успешное получение заказа"):
            assert get_response.status_code == 200
            response_body = get_response.json()
            assert "order" in response_body
            assert isinstance(response_body["order"], dict)
            assert response_body["order"]["track"] == track
    
    @pytest.mark.order
    @allure.title("Получение заказа без номера")
    @allure.description("Проверка получения заказа без номера")
    def test_get_order_by_track_without_track(self, api_client):
        """Проверка получения заказа без номера"""
        with allure.step("Попытаться получить заказ без номера"):
            response = api_client.get_order_by_track("")
        
        with allure.step("Проверить ошибку недостаточных данных"):
            assert response.status_code == 400
            assert_insufficient_data_error(response)
    
    @pytest.mark.order
    @allure.title("Получение заказа с несуществующим номером")
    @allure.description("Проверка получения заказа с несуществующим номером")
    def test_get_order_by_nonexistent_track(self, api_client):
        """Проверка получения заказа с несуществующим номером"""
        with allure.step("Попытаться получить заказ с несуществующим номером"):
            fake_track = 999999999
            response = api_client.get_order_by_track(fake_track)
        
        with allure.step("Проверить ошибку не найденного заказа"):
            assert response.status_code == 404
            assert_order_not_found_error(response)
