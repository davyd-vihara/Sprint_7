...
class TestCreateOrder:
    ...
    def test_create_order_without_color(self, api_client, order_data):
    ...
    @allure.description("Проверка создания заказа с разными цветами (BLACK, GREY, оба)")
    def test_create_order_with_different_colors(self, api_client, order_data, color):
        """Проверка создания заказа с разными цветами (BLACK, GREY, оба)"""
        with allure.step(f"Создать заказ с цветом {color}"):
            response = api_client.create_order_with_color(
                first_name=order_data["first_name"],
                last_name=order_data["last_name"],
                address=order_data["address"],
                metro_station=order_data["metro_station"],
